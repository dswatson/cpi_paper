...
# Registry ----------------------------------------------------------------
reg_name <- "sim_regr_toeplitz_highdim"
...
cpi <- function(data, job, instance, learner_name, ...) {
  par.vals <- switch(learner_name, 
                     ...
                     regr.nnet = list(size = 20, decay = .1, 
                                      trace = FALSE, MaxNWts = 1e6), 
...
plots_cpi <- lapply(unique(res$p), function(pp) {
  res_pp <- res[p == pp, ]
  res_pp[, Variable := droplevels(Variable)]
  levels(res_pp$Variable) <- 1:pp
  res_pp[, Variable2 := as.numeric(Variable)]
  ggplot(res_pp, aes(x = Variable2, y = CPI, group = Variable)) + 
...
names(plots_cpi) <- unique(res$p)

# Histograms of t-test statistics (only null variables)
plots_tstat <- lapply(unique(res$p), function(pp) {
  ggplot(res[p == pp & test == "t" & Variable %in% c("X1"), ], aes(Statistic)) +
...
names(plots_tstat) <- unique(res$p)

# Power (mean over replications)
plots_power <- lapply(unique(res$p), function(pp) {
  res_pp <- res[p == pp, ]
  res_pp[, reject := p.value <= 0.05]
  res_mean <- res_pp[, .(power = mean(reject, na.rm = TRUE)), by = .(Problem, algorithm, Learner, test, Variable, measure)]
  res_mean[, Variable := droplevels(Variable)]
  ...
  ggplot(res_mean, aes(x = Variable, y = power, col = Learner, shape = Learner, linetype = Test)) +
...
names(plots_power) <- unique(res$p)

# Plot all in one plot
library(cowplot)
lapply(as.character(unique(res$p)), function(pp) {
  p <- plot_grid(plots_cpi[[pp]], plots_tstat[[pp]], plots_power[[pp]], 
                 labels = "AUTO", ncol = 1)
  ggplot2::ggsave(paste0(reg_name, "_", pp, ".pdf"), plot = p, width = 10, height = 12)
  ggplot2::ggsave(paste0(reg_name, "_", pp, ".png"), plot = p, width = 10, height = 12, dpi = 300)
})
