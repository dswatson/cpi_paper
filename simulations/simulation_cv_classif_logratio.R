...
# Registry ----------------------------------------------------------------
reg_name <- "cpi_sim_cv_classif_logratio"
...
lapply(unique(res$measure), function(m) {
  p <- plot_grid(plots_cpi[[m]], plots_tstat[[m]], plots_power[[m]], 
                 labels = "AUTO", ncol = 1)
  ggplot2::ggsave(paste0("cv_classif_logratio_", m, ".pdf"), plot = p, width = 10, height = 15)
  ggplot2::ggsave(paste0("cv_classif_logratio_", m, ".png"), plot = p, width = 10, height = 15, dpi = 300)
})
