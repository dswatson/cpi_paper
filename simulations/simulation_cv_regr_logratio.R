...
lapply(unique(res$measure), function(m) {
  p <- plot_grid(plots_cpi[[m]], plots_tstat[[m]], plots_power[[m]], 
                 labels = "AUTO", ncol = 1)
  ggplot2::ggsave(paste0("cv_regr_logratio_", m, ".pdf"), plot = p, width = 10, height = 13)
  ggplot2::ggsave(paste0("cv_regr_logratio_", m, ".png"), plot = p, width = 10, height = 13, dpi = 300)
})
...
invisible(lapply(unique(res$measure), function(m) {
  ...
  print(xtable(tab_m, digits = 4, caption = paste("Regr logratio", m)), booktabs = TRUE, table.placement = "htbp",
